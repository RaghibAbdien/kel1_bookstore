...
class PaymentController extends Controller
{
    ...
    public function virtualStore(Request $request)
    {
        ...
        try {
            ...
            return response()->json([
                'success' => 'Payment and products saved successfully.',
                'redirect' => route('show-virtual-invoice', ['id' => $payment->id])
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function showVirtualInvoice($id)
    {
        $payment = Payment::findOrFail($id);
        $payment_products = PaymentProduct::where('payment_id', $id)->get();
        ...
        return view('virtual_sale.invoice', compact('payment', 'payment_products', 'variant_product', 'tax', 'discount'));
    }  
